...
CREATE OR REPLACE TABLE raw.station_5min (
    ...
    lane_type VARCHAR,  -- A string indicating the type of lane. Possible values and their meaning:
    -- CD (Coll/Dist)
...
CREATE OR REPLACE TABLE raw.station_meta (
    station BIGINT, -- An integer value that uniquely indenties the Station Metadata. Use this value to 'join' other clearinghouse files that contain Station Metadata
    freeway BIGINT,  -- Freeway #
    direction VARCHAR, -- Freeway Direction: A string indicating the freeway direction.
    district BIGINT, -- District #
    county BIGINT, -- County Identifier: The unique number that identifies the county that contains this census station within PeMS.
    city BIGINT, -- City Identifier: The unique number that identifies the city that contains this census station within PeMS.
    state_pm VARCHAR, -- State Postmile.
    abs_pm VARCHAR, -- Absolute Postmile.
    latitude DOUBLE, -- Latitude.
    longitude DOUBLE, -- Longitude.
    length DOUBLE, -- Length of the cencus station
    lane_type VARCHAR, -- A string indicating the type of lane. Possible values and their meaning:
    -- CD (Coll/Dist)
    -- CH (Conventional Highway)
    -- FF (Fwy-Fwy connector)
    ...
    -- ML (Mainline)
    -- OR (On Ramp)
    lanes BIGINT, -- Total number of lanes.
    name VARCHAR, -- Name.
    user_id_1 BIGINT, -- User-entered string identifier 1.
    user_id_2 BIGINT, -- User-entered string identifier 2.
    user_id_3 BIGINT, -- User-entered string identifier 3.
    user_id_4 BIGINT -- User-entered string identifier 4.
);

CREATE OR REPLACE TABLE raw.chp_incident_day (
    incident_id BIGINT,
    cc_code VARCHAR,
    incident_num BIGINT,
    timestamp TIMESTAMP,
...
CREATE OR REPLACE TABLE raw.chp_incident_det_day (
    incident_id BIGINT,
    detail_id BIGINT,
    timestamp TIMESTAMP,
    description VARCHAR
